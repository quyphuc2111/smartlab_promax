...
const SystemConfig: React.FC = () => {
  ...
  const mockYears: SchoolYear[] = [
    { school_year_id: 1, school_year_name: '2023-2024', start_date: '2023-09-05', end_date: '2024-05-31' },
    { school_year_id: 2, school_year_name: '2024-2025', start_date: '2024-09-05', end_date: '2025-05-31' },
  ];

  const mockSlots: PracticeTimeSlot[] = [
    { practice_time_slot_id: 1, practice_time_slot_name: 'Ca Sáng 1', school_year_id: 1, start_time: '07:30', end_time: '09:00' },
    { practice_time_slot_id: 2, practice_time_slot_name: 'Ca Sáng 2', school_year_id: 1, start_time: '09:15', end_time: '10:45' },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">Cấu hình tham số hệ thống theo năm học.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {activeTab === 'years' && mockYears.map((y, idx) => (
                <tr key={y.school_year_id} className="hover:bg-slate-50 transition group">
                  <td className="px-6 py-4 text-sm text-slate-400 font-mono">{idx + 1}</td>
                  <td className="px-6 py-4 font-bold text-slate-700">{y.school_year_name}</td>
                  <td className="px-6 py-4">
                    <div className="flex gap-4">
                      <div className="text-xs"><span className="text-slate-400">Bắt đầu:</span> <span className="font-semibold">{y.start_date}</span></div>
                      <div className="text-xs"><span className="text-slate-400">Kết thúc:</span> <span className="font-semibold">{y.end_date}</span></div>
                    </div>
                  </td>
              ...
              {activeTab === 'slots' && mockSlots.map((s, idx) => (
                <tr key={s.practice_time_slot_id} className="hover:bg-slate-50 transition group">
                  <td className="px-6 py-4 text-sm text-slate-400 font-mono">{idx + 1}</td>
                  <td className="px-6 py-4 font-bold text-slate-700">{s.practice_time_slot_name}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded text-xs font-bold">{s.start_time}</span>
                      <ArrowRightLeft className="w-3 h-3 text-slate-300" />
                      <span className="px-2 py-1 bg-rose-50 text-rose-600 rounded text-xs font-bold">{s.end_time}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <RowActions />
