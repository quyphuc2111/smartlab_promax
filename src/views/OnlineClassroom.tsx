...
import { 
  ...
  Users, MicOff, VideoOff,
  ScreenShare, X, Send,
  Bot
} from 'lucide-react';
import { User, Message, UserRole } from '../types';

const OnlineClassroom: React.FC<{ user: User }> = ({ user }) => {
  const [isTeaching, setIsTeaching] = useState(false);
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', senderId: 'ai', senderName: 'LabMaster AI', content: 'Chào thầy cô, tôi là trợ lý AI. Tôi có thể giúp gì cho ca dạy hôm nay?', timestamp: '14:00', role: UserRole.ADMINISTRATOR }
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // If starts with @ai, simulate AI response
    if (inputMessage.toLowerCase().startsWith('@ai')) {
      ...
      setTimeout(() => {
        const aiMsg: Message = {
          ...
          content: 'Tôi đã nhận được câu hỏi của bạn. Hãy để tôi xử lý...',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    <div className="h-[calc(100vh-10rem)] flex flex-col lg:flex-row gap-6">
          ...
          {isTeaching ? (
            <div className="w-full h-full relative">
              ...
              <div className="absolute bottom-6 right-6 w-48 aspect-video bg-slate-800 rounded-xl border-2 border-white/20 overflow-hidden shadow-2xl">
                 <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.user_name}`} className="w-full h-full object-cover" alt="Teacher" />
      ...
      <div className="w-full lg:w-80 flex flex-col gap-4">
        <div className="flex-1 bg-white rounded-3xl border border-slate-200 flex flex-col overflow-hidden shadow-sm">
          ...
          >
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.senderId === user.user_id.toString() ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`
                  ...
                  ${msg.senderId === user.user_id.toString() 
